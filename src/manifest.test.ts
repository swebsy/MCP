...
describe("Claude Code plugin manifest", () => {
  ...
  // Both are regenerated by scripts/mirror-mcp.sh, but the community catalogue
  // pins the mirror at a commit SHA and only offers users an update when this
  // version moves — so a stale one ships silently rather than failing loudly.
  it("pins the same version as the npm package", () => {
    expect(plugin.version).toBe(pkg.version);
  });

  it("installs the published package, not a local build", () => {
    expect(mcp.mcpServers.swebsy).toEqual({
      command: "npx",
      args: ["-y", pkg.name],
    });
  });
